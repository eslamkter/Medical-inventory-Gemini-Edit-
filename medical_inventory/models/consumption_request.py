...
class ConsumptionRequest(models.Model):
    ...
    def action_process_stock(self):
        ...
        return {
            ...
            'res_model': 'medical.consumption.request',
            'res_id': self.id,
            'view_mode': 'form',
            'target': 'current',
        }
    ...
    def _do_transfer(self):
        ...
        for line in self.line_ids:
            if line.product_id.type not in ('product', 'storable'):
                continue
            ...
            if available < line.quantity:
                raise UserError(_(
                    'Not enough stock for %s. Available: %.2f, Requested: %.2f'
        ...
        self.message_post(body=_('Stock transferred from %s to %s.') % (
            self.source_location_id.name, self.department_location_id.name))
    ...
    def _do_consumption(self):
        for line in self.line_ids:
            ...
            if available < line.quantity:
                raise UserError(_(
                    'Not enough stock for %s in %s. Available: %.2f, Consumed: %.2f'
                ) % (line.product_id.name, self.department_location_id.name,
                     available, line.quantity))
            if quant:
                quant.sudo().write({'quantity': max(0, quant.quantity - line.quantity)})
        self.message_post(body=_(
            'Consumption recorded. Stock reduced in %s.') % self.department_location_id.name)
    ...
    def get_dashboard_data(self):
        ...
        expiry_items = [{
            ...
            'days_left': (l.expiry_date.date() - today).days if l.expiry_date else 0,
            'location': l.receive_id.destination_location_id.name or '',
            'expired': (l.expiry_date.date() - today).days < 0 if l.expiry_date else False,
            'critical': 0 <= (l.expiry_date.date() - today).days <= 7 if l.expiry_date else False,
